...
for (let product of searchdata)
{
    test(`verify the product header name ${product.productname}`, {tag:['@sanity', '@regression']}, async({homepage})=>{
        ...
        let resultpage:ResultsPage = await homepage.searchItems(product.searchkey);
        let productinfo:ProductInfoPage = await resultpage.selectProduct(product.productname);
